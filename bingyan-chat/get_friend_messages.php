...
if (empty($friend)) {
    echo '<div style="text-align:center;padding:20px;color:#e53e3e;">参数错误</div>';
    exit();
}
...
foreach ($friends as $f) {
    if (($f['user1'] === $currentUser && $f['user2'] === $friend) ||
        ($f['user1'] === $friend && $f['user2'] === $currentUser)) {
        $isFriend = true;
        break;
    }
}
...
if (!$isFriend) {
    echo '<div style="text-align:center;padding:20px;color:#e53e3e;">你不是对方的好友</div>';
    exit();
}
...
foreach ($pageMessages as $msg) {
    ...
    if (!empty($msg['recalled'])) {
        ...
        if (!empty($msg['message'])) echo htmlspecialchars($msg['message']);
        if (!empty($msg['image_url'])) echo '<img class="chat-image" src="' . htmlspecialchars($msg['image_url']) . '" alt="图片" loading="lazy">';
    }
    echo '</div>';
